...
def main_worker(gpu, ngpus_per_node, args):
    ...
    print(gen_net)
    dis_net = Discriminator(in_channels=6, patch_size=15, data_emb_size=50, label_emb_size=10, seq_length = 60, depth=3, n_classes=6)
    print(dis_net)
    ...
    # train loop
    t=0
    print(f'Epochs between checkpoint: {30}')
    for epoch in range(int(start_epoch), int(args.max_epoch)):
        ...
        if epoch % 30 == 0:
            print('\n\n')
            save_checkpoint({'epoch':epoch,'gen_state_dict': gen_net.module.state_dict(), 'dis_state_dict': dis_net.module.state_dict()}, False, args.path_helper['ckpt_path'], filename=f"{t}_checkpoint")
